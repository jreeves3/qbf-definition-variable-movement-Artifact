...
echo "DETECT"
timeout 100s ./../build/kissat temp.cnf --gateextraction=1 --gatexorrecurse=1
mv "kissat_gates.gate" $2"/"$(basename $1 .qdimacs)".gate"

echo "MOVE"
python3 ../qbf-gate-processor.py -q $1 -g $2"/"$(basename $1 .qdimacs)".gate" -p temp.qrat -o $2"/"$(basename $1 .qdimacs)".qdimacs" -m
