...
{
  start_search (solver);
  if (GET_OPTION (gateextraction)) {
    solver->gates_file = fopen("kissat_gates.gate", "w+");
    fprintf(solver->gates_file,"c Gate Types:\n");
    fprintf(solver->gates_file,"c 0: exotic\n");
    fprintf(solver->gates_file,"c 1: equivalence\n");
    fprintf(solver->gates_file,"c 2: and\n");
    fprintf(solver->gates_file,"c 3: or\n");
    fprintf(solver->gates_file,"c 4: xor\n");
    fprintf(solver->gates_file,"c 5: ite\n\n");
    kissat_eliminate (solver);
    return(0);
  }
    //return 0;
